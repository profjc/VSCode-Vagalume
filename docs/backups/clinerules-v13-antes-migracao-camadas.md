...
<!-- ║  PRIORIDADE: regra mais recente e específica (N1.2) — lições N3.6     ║ -->
<!-- ║  atualizam/refinam N1–N2 em caso de divergência.                       ║ -->
<!-- ╚══════════════════════════════════════════════════════════════════════════╝ -->
...
## N1.0 Regras de Bolso (leitura obrigatória — se ler só isto, leia isto)
Regras inegociáveis, consolidadas do corpo do arquivo. Em caso de conflito, prevalecem as regras mais recentes e específicas (ver N1.2).

1. **Tool call limpa**: toda chamada de ferramenta (write_to_file, replace_in_file, execute_command, etc.) deve ser o **primeiro e único conteúdo** da mensagem — nunca precedida de texto, markdown ou `<thinking>` (N1.9.1).
2. **Checkpoint primeiro**: ao iniciar sessão, ler `Onde-paramos.md` antes de qualquer ação (N1.7).
3. **PLAN planeja, ACT executa**: uma página/arquivo por ciclo, com validação do mestre entre ciclos — nunca em lote (N1.3.3, N1.4.1).
4. **Escopo estrito**: fazer exatamente o solicitado, sem extrapolar, "padronizar" ou corrigir o que não foi pedido (N1.5).
5. **Foco ao VS Code**: ao final do ciclo de uma página a copiar, `code -r <caminho>` deve ser a **ÚLTIMA ação** (nunca seguido de atualizações de checkpoint) (N3.6).
6. **Método padrão de imagem**: sempre placeholder com URL fixa + imagem oficial `src="[cole a imagem aqui]"` + marcador "⚠️ APAGAR ESTE BLOCO" — nunca `src=""`, `@@PLUGINFILE@@` ou data URI (N2.1.1.3, N3.6).
7. **Sem `<style>`/`<link>`/`<script>` globais** no HTML entregue; JS apenas local com `addEventListener` quando estritamente necessário (N2.1.1, N2.1.2).
8. **Acessibilidade desde o início**: padrão N2.4.7 (descrição longa sem duplicidade), atributos de imagem `width`/`height`/`loading="lazy"`, hierarquia de headings, sem VLibras (N2.4.12).
9. **Navegação via Moodle**: botões "Anterior"/"Próximo"/"Finalizar" são configurados no Moodle, nunca no HTML (N3.5.1).
10. **Auto-auditoria antes de entregar**: conferir a lista N1.10 antes de qualquer `attempt_completion`.
...
Antes de QUALQUER ação (gerar, editar, decidir), ler este arquivo por inteiro (ou, no mínimo, as "Regras de Bolso" N1.0). Esta regra tem prioridade máxima sobre todas as outras. **Em caso de conflito entre regras, prevalece a mais recente e específica** — as lições aprendidas de N3.6 atualizam/refinam as regras N1–N2 quando houver divergência (ex: o método `@@PLUGINFILE@@` citado em versões antigas foi substituído pelo método padrão — ver N2.1.1.3).
...
## N1.10 Auto-auditoria de conformidade (obrigatória antes de qualquer entrega)
Antes de cada `attempt_completion`, conferir rapidamente a conformidade com estas regras (30 segundos):

- [ ] **Tool call limpa**: mensagem iniciou com o XML da ferramenta, sem texto prévio (N1.9.1)
- [ ] **Escopo estrito**: entreguei exatamente o solicitado, sem extrapolações (N1.5)
- [ ] **Um arquivo por ciclo**: nenhum lote; página/arquivo único conforme o ciclo PLAN → ACT → validação (N1.3.3)
- [ ] **Foco ao VS Code**: se era página a copiar no Moodle, `code -r` foi a ÚLTIMA ação do ciclo (N3.6)
- [ ] **Checkpoint atualizado**: `Onde-paramos.md` reflete o trabalho concluído (N1.6.2)
- [ ] **Página validada**: conferida contra `docs/checklist-entrega.md` (N3.5.2)
