...
3. **Placeholder de imagem via `src=""` vazio (solução definitiva validada 06/09/2026 para TinyMCE 4.5/Trema)**: Após configuração do Moodle habilitada pelo mestre, o TinyMCE 4.5/Trema **aceita `src=""`** no `<img>` do bloco temporário (mostra um quadradinho vazio de marcação; a URL fixa não é mais necessária — resolve o problema do link-morto ao mover o arquivo). O bloco temporário usa `src=""` num `<img>` 40×40, delimitado por comentários `INÍCIO`/`FIM DA IMAGEM TEMPORÁRIA`, junto ao marcador `⚠️ APAGAR ESTE BLOCO` e à imagem oficial com `src="[cole a imagem aqui]"`. **Referência canônica do bloco: `templates/pages/M2/Parte4/Licao3/M2P4L3p1.html`** (sempre copiar o bloco real de uma página, nunca reconstruir de cabeça). **Aplica-se APENAS a imagens**: H5P (placeholder nativo `[LINK_DO_ARQUIVO_.H5P]`), vídeos, áudios e iframes ficam fora deste padrão. Bloco padrão:
   ...
   <img src="" alt="Upload temporario no TinyMCE" width="40" height="40" style="border-radius: 4px; flex-shrink: 0;"><br>
   <!-- ===== FIM DA IMAGEM TEMPORÁRIA ===== -->
   ```

   **Fluxo de uso no Moodle:** colar HTML → clicar no quadradinho vazio (imagem temporária) → o TinyMCE abre o gerenciador de arquivos → selecionar o arquivo real → o Tiny substitui o `src` pelo `pluginfile.php` real → **o que antes era `src=""` agora tem a URL** → **apagar o bloco temporário** (do marcador `⚠️ APAGAR ESTE BLOCO` até o comentário `FIM`, localizável via Ctrl+F "APAGAR"). A imagem oficial mantém `alt`, dimensões, `sr-only` e `style` de exibição intactos. **IMPORTANTE:** o padrão vale para **imagens**; vídeos/H5P seguem seus próprios placeholders.
...
> **Regra (corrigida 04/09/2026): títulos de cards/caixas usam `<p class="font-weight-bold">`, não classe `.h5`/`.h6`.** O padrão do projeto para títulos dentro de caixa/card é `<p class="font-weight-bold mb-0" style="line-height:1.2;">` — negrito via classe, herdando o marrom `#5B3925` do bloco (que o CSS global força com `!important` no `.vagalume-destaque-bloco`). A classe Bootstrap `.h5`/`.h6` em `<strong>`/`<span>`/`<p>` deve ser evitada em títulos de cards (muda o padrão) — se encontrada, converter para `p.font-weight-bold`. **Única exceção legítima:** o título FIXO "Para participar" do template de fórum, que mantém `span.h5 font-weight-bold` + `color:#5b3925` (elemento fixo de template). **Precedente corrigido:** `M1P4L1p3.html` tinha `strong.h5` nos cards "Pessoas cuidadoras" — convertido para `p.font-weight-bold`, alinhado aos cards de referência "Colocando em prática" (`M2P2L1p2.html`).
